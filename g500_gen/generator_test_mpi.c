...
int main(int argc, char** argv) 
{
	...
	if(rank==0) 
        fprintf(stdout,"Input: /path/to/exe log_numverts degree row-partitions column-partitions\n");
	...
	//exit(-1);
    if (rank == 0) 
        fprintf(stderr, "Graph size is %" PRId64 " vertices and %" PRId64 " edges\n", INT64_C(1) << log_numverts, degree << log_numverts);
...
/*
	...
	for(i=0;i<my_edges;++i)
	{
		uint32_t src=result[i].v0_low;
		uint32_t des=result[i].v1_low;
