...
void generate_kronecker_range(
  ...
  sprintf(filename,"scale-%lu-rank-%d.bin",logN, rank);
...
#pragma mta block schedule
  ...
  int64_t edge_count = (1L << 20);
  int64_t x = start_edge;
  packed_edge* buf = (packed_edge*)malloc(sizeof(packed_edge)*edge_count);
  packed_edge pedge;
  printf("rank = %d, start_edge = %ld, end_edge=%d\n", rank, start_edge, end_edge);
 
  for (ei = start_edge; ei < end_edge; ++ei) {
    mrg_state new_state = state;
    mrg_skip(&new_state, 0, ei, 0);
    make_one_edge(nverts, 0, logN, &new_state, &pedge, val0, val1);
    buf[ei - x] = pedge;
    if ((ei - x ) == (edge_count - 1)) {
        fwrite(buf, sizeof(packed_edge), edge_count, f);
        x = ei + 1;
    }
  }
  if (ei != x ) {
        fwrite(buf, sizeof(packed_edge), ei - x, f);
  }
  fclose(f);
